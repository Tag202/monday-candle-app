...
class MondayService {
    ...
    static async getBoardColumns(token: string, boardId: number) {
        ...
        const response = await mondayClient.request<any>(query, variables);

        return response?.boards?.[0]?.columns ?? [];
    }

    static async createOrderItem( token, boardId, groupId, itemName, columnValues: Record<string, string | number> ) {
        const mondayClient = new ApiClient({ token });

        const created = await mondayClient.operations.createItemOp({
        ...
        for (const columnId in columnValues) {
            await mondayClient.operations.changeColumnValueOp({
                boardId: boardId,
                itemId: itemId,
                columnId: columnId,
                value: JSON.stringify(columnValues[columnId])
            });
        }
